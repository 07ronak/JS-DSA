...
var canCompleteCircuit = function (gas, cost) {
  // Step 1: Check if the total gas available is less than the total cost.
  // If true, a complete trip is impossible, so return -1.
  if (
  ...
  let total = 0; // Tracks the fuel level
  let res = 0; // Stores the starting station index

  // Step 2: Iterate through each gas station
  for (let i = 0; i < gas.length; i++) {
    total += gas[i] - cost[i]; // Calculate fuel left after visiting this station

    // Step 3: If fuel goes negative, reset the start station to the next one
    if (total < 0) {
      total = 0; // Reset fuel tracker
      res = i + 1; // Set next station as the potential start
    }
  }

  // Step 4: Return the calculated starting station
  return res;
};
