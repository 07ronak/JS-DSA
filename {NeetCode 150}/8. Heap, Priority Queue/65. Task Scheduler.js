/* You are given an array of CPU tasks, each labeled with a letter from A to Z, and a number n. Each
 CPU interval can be idle or allow the completion of one task. Tasks can be completed in any order, but
  there's a constraint: there has to be a gap of at least n intervals between two tasks with the same label.
...
function leastInterval(tasks, n) {
  let freq = {}; // Use an object to count task frequencies
  ...
  // Return the maximum of intervals and task length
